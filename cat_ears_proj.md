...
![Image of the planned components]()

I had a lot of [Dollar Tree Foam Board]() lying around, and wasn't really in the mood to CAD things for 3D printing. This would also allow me to start building without knowing exact dimensions, which is great for organic shapes like these. The sculpting process is simpler and got the job done, live on stream.

![Gif from stream of cutting some foam]()
...
![Gif from the stream of me struggling with the program]()

However. As I said earlier, beat detection is hard. Even on a dead-simple song like [Benny Benassi - *Satisfaction*](), finding the right bass spread took a lot of fiddling with the values. And the worst part is that changes by song (as producers mix the bass differently) and even depending on what speaker is being used to play the music.

{% spotify_embed id= %}

![Gif of the ears behaving erratically to a standard beat. maybe a youtube clip?]()
...
![Gif of the performance on the desk]()
